...
class StringsTest(unittest.TestCase):
    ...
    def test_contains_with_matching_patterns(self):
        ...
        assert contains('afgd', 'afgd') is True
        assert contains('aaa', 'a') is True  # multiple occurrences
        assert contains('ababab', 'bab') is True
        assert contains('aaa', 'aa') is True  # overlapping pattern
        assert contains('axaxax', 'axa') is True
        assert contains('aXaa', 'Xa') is True
    ...
    def test_contains_with_non_matching_patterns(self):
        ...
        assert contains('abc', 'bcc') is False
        assert contains('abc', 'abcde') is False
        assert contains('abc', 'cab') is False
    ...
    def test_find_index_with_matching_patterns(self):
        ...
        assert find_index('axaxax', 'axa') == 0
        assert find_index('ababab', 'bab') == 1
        assert find_index('afgd', 'afgd') == 0
        assert find_index('aXaa', 'Xa') == 1
        assert find_index('aasdfasdfadsfaax', 'aax') == 13
    ...
    def test_find_index_with_complex_patterns(self):
        ...
        assert find_index('sap sap', ' sap') == 3
        assert find_index('abcd eabcde', 'abcde') == 6
        # TODO: Write more test cases that check complex patterns or edge cases
        # You'll need a lot more than this to test your algorithm's robustness
    ...
    def test_find_all_indexes_with_matching_patterns(self):
        ...
        assert find_all_indexes('axaxax', 'axa') == [0, 2]
        assert find_all_indexes('ababab', 'bab') == [1, 3]
        assert find_all_indexes('afgd', 'afgd') == [0]
        assert find_all_indexes('aXaa', 'Xa') == [1]
    ...
    def test_find_all_indexes_with_non_matching_patterns(self):
        ...
        assert find_all_indexes('abc', 'bcc') == []
        assert find_all_indexes('abc', 'abcde') == []
        assert find_all_indexes('abc', 'cab') == []
    ...
    def test_find_all_indexes_with_complex_patterns(self):
        ...
        assert find_all_indexes('sap sap', ' sap') == [3]
        assert find_all_indexes('wiwiwiwiwiwi', 'wiwiwiwi') == [0, 2, 4]
        # TODO: Write more test cases that check complex patterns or edge cases
        # You'll need a lot more than this to test your algorithm's robustness
